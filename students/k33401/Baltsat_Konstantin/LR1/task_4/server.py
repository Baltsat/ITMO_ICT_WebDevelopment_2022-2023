...
def handle_client(socket, address, username):
    if not clients.setdefault(socket):
        ...
        send_message(
            socket, f'Пользователь {username} присоединился к чату'.encode())

    while True:
        try:
            message = socket.recv(1024)
            if message.decode('utf-8').find('EXIT') != -1:
                print(f'{username}[{address[0]}:{address[1]}] покинул чат')
                send_message(
                    socket, f'Пользователь {username} покинул чат'.encode())
                clients.pop(socket)
                break
            send_message(socket, message)
        except ConnectionResetError:
            print(
                f'{username}[{address[0]}:{address[1]}] пропал.')
            send_message(
                socket, f'Пользователь {username} пропал.'.encode())
            break
    socket.close()
